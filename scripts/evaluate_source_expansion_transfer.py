"""Replay all fixed R9 source procedures on already-used target forecast banks."""
...
def main():
    ...
    for name, path in {
        "study-root": "artifacts/iclr27-r9/source-expansion-learning-v001",
        "study-audit": "artifacts/iclr27-r9/source-expansion-audit-v001",
        ...
        "protocol": "docs/iclr2027/R9_TRANSFER_PROTOCOL.md",
    }.items():
    ...
    studies = {
        "r9": (args.study_root, read_json(args.study_root / "manifest.json")),
        "r7": (args.reference_root, read_json(args.reference_root / "manifest.json")),
    }
    for name, audit_root in (("r9", args.study_audit), ("r7", args.reference_audit)):
    ...
    for model_id in ("chronos2", "timesfm2p5"):
        ...
        configurations = [
            ("r9", f"{regime}_{label}", regime, label)
            for regime in ("expanded", "steps_matched")
            for label in ("teacher", "future")
        ...
        for study_name, method, regime, label in configurations:
            ...
            entries = [
                ...
                and (
                    row.get("regime") == regime and row.get("label") == label
                    if study_name == "r9"
            ...
            for entry in entries:
                ...
                if len(set(saved["training_origins"])) != (322 if regime == "expanded" else 165):
        ...
        for cohort, (_, prep, horizons) in cohorts.items():
            ...
            for horizon in horizons:
                ...
                for source_name, (root, _) in studies.items():
                    controls = read_json(root / model_id / "full_source/controls.json")
                    for label in ("teacher", "future"):
