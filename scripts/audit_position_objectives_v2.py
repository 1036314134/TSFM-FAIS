...
def main():
    ...
    recovery = study["reporting_recovery"]
    if recovery["script_sha256"] != file_sha256(
        ROOT / "scripts/finish_position_objectives.py"
    ) or recovery["note_sha256"] != file_sha256(
        ROOT / "docs/iclr2027/R17_REPORTING_RECOVERY_NOTE.md"
    ):
        raise ValueError("the reporting recovery changed")
    for entry in recovery["preserved_files"]:
        if file_sha256(args.study_root / entry["path"]) != entry["sha256"]:
            raise ValueError("a pre-recovery artifact changed")
