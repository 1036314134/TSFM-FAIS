"""Apply the pinned MoTM reference to prepared native confirmation histories."""
...
def main():
    ...
    for name in ("prepared-root", "reference-root", "runtime-root", "output-root"):
        parser.add_argument("--" + name, type=Path, required=True)
    args = parser.parse_args()
    output = args.output_root.resolve()
    if (output / "manifest.json").exists():
        raise ValueError("preserve completed native MoTM inputs")
    output.mkdir(parents=True, exist_ok=True)
    prepared = json.loads((args.prepared_root / "manifest.json").read_text(encoding="utf-8"))
    if prepared["status"] != "completed" or len(prepared["episodes"]) != 373:
        raise ValueError("complete the registered confirmation input preparation")
    identity = {
        ...
        "input_recipe": "raw observed histories; published internal normalization; shared LOCF/prefix-median fallback",
    }
    identity_path = output / "identity.json"
    if identity_path.exists() and json.loads(identity_path.read_text(encoding="utf-8")) != identity:
        raise ValueError("native MoTM preparation identity changed")
    ...
    for record in prepared["episodes"]:
        ...
        if file_sha256(source) != record["sha256"]:
            raise ValueError("a prepared native history changed")
        path = output / "episodes" / source.name
        if not path.exists():
            with np.load(source, allow_pickle=False) as saved:
                context = saved["context"]
                actions = saved["candidate_ids"].tolist()
                fallback = saved["candidate_values"][actions.index("locf")]
            ...
            if not np.isfinite(completed).all():
                raise ValueError("the MoTM input is not finite")
            _save_npz(
                path,
                values=completed,
        ...
        with np.load(path, allow_pickle=False) as saved:
            if (
                ...
                raise ValueError("a native MoTM cache has different provenance")
        records.append(
            {
        ...
        _write_json(
            ...
            {"status": "preparing", "completed_episodes": len(records), "total_episodes": 373},
        )
