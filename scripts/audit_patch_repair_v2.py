...
def independent_repair(embedding, missing, state, condition):
    ...
    weights = {name: np.asarray(value, dtype=np.float32) for name, value in state.items()}
    active = missing.any(-1)
    if not active.any():
        return raw.copy()
    centered = raw - raw.mean(-1, keepdims=True)
    normalized = centered / np.sqrt((centered**2).mean(-1, keepdims=True) + np.float32(1e-5))
    descriptor = missing.astype(np.float32)
    if condition == "fraction":
        descriptor = np.broadcast_to(descriptor.mean(-1, keepdims=True), descriptor.shape)
    projected = normalized @ weights["down.weight"].T + descriptor @ weights["mask.weight"].T
    hidden = np.float32(0.5) * projected * (np.float32(1) + erf(projected / np.sqrt(np.float32(2))))
    change = hidden @ weights["up.weight"].T
    return np.where(active[..., None], raw + change, raw)
...
def main():
    ...
    _write_json(
        output / "manifest.json",
        {
            ...
            "previous_audit_sha256": file_sha256(ROOT / "scripts/audit_patch_repair.py"),
            "independent_reconstruction_dtype": "float32",
