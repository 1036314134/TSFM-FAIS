...
def main():
    ...
    parent_inputs = Path(prepared["deep_fits_reused_from"])
    check_hash(parent_inputs / "manifest.json", prepared["parent_input_manifest_sha256"])
    parent_prepared = read_json(parent_inputs / "manifest.json")
    parent_cases = {r["case_id"]: r for r in parent_prepared["cases"]}
    if prepared["deep_fits"] != parent_prepared["deep_fits"]:
        raise ValueError("the original neural baseline artifacts were not reused")
    if prepared["training_batch_sha256"] != parent_prepared["training_batch_sha256"]:
        raise ValueError("the original neural training information changed")
    ...
    if not args.smoke and len(prepared["cases"]) != 864:
    ...
    for row in prepared["cases"]:
        ...
        if row["outage_pattern"] == "targets_only":
            ...
            inherited = load_npz(parent_inputs / old["path"], old["sha256"])
            for key in d:
                np.testing.assert_array_equal(d[key], inherited[key])
        ...
        if row["outage_pattern"] in ("local_pollutants", "regional_pollutants"):
            original[-age:, :6] = np.nan
        if row["outage_pattern"] == "regional_pollutants":
            original[-age:, 11:17] = np.nan
    ...
    _write_json(
        output / "manifest.json",
        {
            ...
            "audit_version": "r32_v001_group_outage_and_fixed_output_layout",
