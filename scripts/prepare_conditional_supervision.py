...
import torch
from conditional_future import condition_history, draw_history
...
def main():
    ...
    parser.add_argument(
        "--protocol",
        type=Path,
        default=ROOT / "docs/iclr2027/R16_CONDITIONAL_SUPERVISION_PROTOCOL.md",
    )
    args = parser.parse_args()
    output = args.output_root.resolve()
    if (output / "manifest.json").exists():
        raise ValueError("preserve completed controlled histories")
    definitions = supervision_scenarios()
    serial = [
        {
    ...
    identity = {
        ...
        "models": serial,
        "prefix_length": 6144,
        "histories_per_process": 20,
        "training_histories_per_process": 16,
        ...
        "seed_scheme": "16000000+100000*stream+1000*generator+10*history+condition; streams prefix=0/calibration_mask=1/history=2/mask=3/imputation=4/factual_future=5",
    }
    ...
    for generator, model in enumerate(definitions):
        ...
        for index, start in enumerate(starts):
            ...
            rng = np.random.default_rng(supervision_seed(1, generator, index))
        ...
        for origin in range(20):
            ...
            clean = draw_history(model, 96, phase, supervision_seed(2, generator, origin))
            for condition, (mechanism, rate) in enumerate(identity["mask_conditions"]):
                context = clean.copy()
                rng = np.random.default_rng(supervision_seed(3, generator, origin, condition))
                ...
                if not path.exists():
                    ...
                    completed = runner.run_many(
                        ...
                        batch,
                        local,
                        seed=supervision_seed(4, generator, origin, condition),
                        params=params,
                    )
                ...
                records.append(
                    {
                        ...
                        "generator": generator,
                        "family_id": model["group"],
                        "dataset_id": model["name"],
                        "item_id": "series",
                        "split": "train" if origin < 16 else "validation",
                        "history": origin,
                        "phase": phase,
                        "mechanism": mechanism,
                        "missing_rate": rate,
                        "condition": condition,
                        "future_seed": supervision_seed(5, generator, origin),
                        "path": str(path.relative_to(output)),
                        "sha256": file_sha256(path),
                    }
                )
        print(f"{model['name']}: independent prefix and 100 observed inputs prepared", flush=True)
        del artifacts
        gc.collect()
    imputer.verify_frozen()
    if (
        len(records) != 1200
        or len({row["origin_id"] for row in records}) != 240
        or len(fitting) != 24
    ):
